...
public class ToggleProductionCommand implements Command{
    ...
    public ToggleProductionCommand(int index) {
        this.index = index;
    }
    ...
    public boolean doAction(Controller c, String nickname) {
        ...
        if (check() && game.getCurrentNickname().equals(nickname)) {
            try {
                if (index == 0)
                    game.toggleBaseProd();
                else game.toggleCardProd(index);
            } catch (IllegalSlotException | IllegalActionException | UnknownFoundException | IndexOutOfBoundsException | NullPointerException e) {
                return false;
            }
            return true;
        } else return false;
